...
void HAL_PCD_MspInit(PCD_HandleTypeDef* pcdHandle)
{
  ...
  /* USER CODE END USB_OTG_FS_MspInit 0 */
    ...
    */
    ...
    /* Priority must be numerically LOWER than configLIBRARY_MAX_SYSCALL_INTERRUPT_PRIORITY (5)
     * so the USB ISR is never masked by FreeRTOS's BASEPRI during critical sections.
     * HAL_PCD_IRQHandler does not call any FreeRTOS API, so it is safe above the syscall limit. */
